...
public class BancoDados {
	...
	public void mudarLoginList(String loginConta, String loginNovo) {
		alterarLoginComunidade(loginConta, loginNovo);
		alterarLoginMensagem(loginConta, loginNovo);
		alterarLoginSolAmi(loginConta, loginNovo);
		alterarLoginAmizade(loginConta, loginNovo);
		alterarLoginFeed(loginConta, loginNovo);
	...
	private void alterarLoginComunidade(String loginConta, String loginNovo) {
		for(Comunidade val : listCom) {
	    	if(loginConta.contentEquals(val.getLoginDono()) == true) {
	    		val.setLoginDono(loginNovo);
	    	} else if (val.buscarMembro(loginConta) == 1){
	    		val.removeMembro(loginConta);
	    		val.addMembro(loginNovo);
	    	} 
	        List <Mensagem> cancConta = new ArrayList<>();
	    	cancConta = val.getMensagens();
	    	for(Iterator<Mensagem> iterator2 = cancConta.iterator(); iterator2.hasNext();) {
	    		Mensagem val3 = iterator2.next();
	    		if(val3.getLoginItinerario().contentEquals(loginConta)) {
	    			val3.setLoginItinerario(loginNovo);
	    		}
	    	}
	    }
	}
	
	private void alterarLoginMensagem(String loginConta, String loginNovo) {
		for(Mensagem val : listMenUsu) {
	    	if(loginConta.contentEquals(val.getLoginItinerario()) == true) {
	    		val.setLoginItinerario(loginNovo);
	    	} else if(loginConta.contentEquals(val.getLoginDestinatario()) == true) {
	    		val.setLoginDestinatario(loginNovo);
	    	}
	    }
	}
	
	private void alterarLoginSolAmi(String loginConta, String loginNovo) {
		for(Amizades val : listSolAmi) {
			if(loginConta.contentEquals(val.getUsuario1()) == true) {
	   			for(Conta val2 : list) {
	   				if(val.getUsuario2().contentEquals(val2.getLogin())) {
	   					val2.removeSolAmi(loginConta);
	   					val2.addSolAmi(loginNovo);
	    			}
	   			}
	   			val.setUsuario1(loginNovo);
	   		} else if(loginConta.contentEquals(val.getUsuario2()) == true) {
	   			for(Conta val3 : list) {
	   				if(val.getUsuario1().contentEquals(val3.getLogin())) {
	   					val3.removeSolAmi(loginConta);
	   					val3.addSolAmi(loginNovo);
	    			}
	    		}
	   			val.setUsuario2(loginNovo);
	    	}
		}
	}
	
	private void alterarLoginAmizade(String loginConta, String loginNovo) {
		for(Amizades val : listAmi) {
	   		if(loginConta.contentEquals(val.getUsuario1()) == true) {
	   			for(Conta val2 : list) {
	   				if(val.getUsuario2().contentEquals(val2.getLogin())) {
	   					val2.removeAmigos(loginConta);
	   					val2.addAmigos(loginNovo);
	    			}
	   			}
	   			val.setUsuario1(loginNovo);
	   		} else if(loginConta.contentEquals(val.getUsuario2()) == true) {
	   			for(Conta val3 : list) {
	   				if(val.getUsuario1().contentEquals(val3.getLogin())) {
	   					val3.removeAmigos(loginConta);
	   					val3.addAmigos(loginNovo);
	    			}
	    		}
	   			val.setUsuario2(loginNovo);
	    	}
	...
	private void alterarLoginFeed(String loginConta, String loginNovo) {
		for(Feed val : listFeed) {
	   		if(loginConta.contentEquals(val.getLogin()) == true) {
	   			val.setLogin(loginNovo);
	   		}
